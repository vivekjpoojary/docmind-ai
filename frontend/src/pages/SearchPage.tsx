    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div>
        <div className="flex items-center space-x-2">
          ...
          <h1 className="text-2xl font-bold font-display text-slate-100">Hybrid Passage Search Engine</h1>
        </div>
      ...
      <form onSubmit={handleSearch} className="space-y-4">
        <div className="flex flex-col sm:flex-row items-center gap-3">
          <div className="relative flex-1 w-full">
            <Search className="w-5 h-5 absolute left-4 top-1/2 -translate-y-1/2 text-cyber-cyan" />
            <input
              type="text"
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder="Search for terms, topics, or exact text..."
              className="glass-input w-full pl-11 pr-4 py-3.5 rounded-2xl text-slate-100 font-sans text-sm focus:outline-none"
            />
          </div>
          ...
          <button
            ...
            className="w-full sm:w-auto px-6 py-3.5 bg-gradient-to-r from-cyber-cyan to-cyber-indigo text-white font-semibold text-xs rounded-2xl shadow-[0_0_20px_rgba(6,182,212,0.35)] disabled:opacity-50 transition-all shrink-0"
          >
          ...
          {(['hybrid', 'semantic', 'keyword'] as const).map((m) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-xl capitalize transition-all ${
                mode === m
                  ? 'bg-cyber-cyan/20 text-cyber-cyan border border-cyber-cyan/40 font-bold shadow-[0_0_10px_rgba(6,182,212,0.2)]'
      ...
      <div className="space-y-4">
        {searched && (
          <p className="text-xs font-mono text-slate-400">
            Found <span className="text-cyber-cyan font-bold">{results.length}</span> matching passage(s) using <span className="capitalize font-bold text-white">{mode}</span> mode.
          </p>
        )}
        ...
        {results.length === 0 && searched ? (
          <div className="glass-panel rounded-2xl p-12 text-center border border-white/10 space-y-2">
            ...
            <p className="text-xs font-mono text-slate-500">Try adjusting your query or switching to Hybrid search mode.</p>
          </div>
        ) : (
            ...
            {results.map((res, idx) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] font-mono font-bold px-2.5 py-0.5 rounded-full bg-cyber-cyan/15 text-cyber-cyan border border-cyber-cyan/30">
